...
> 처음 사용하시나요? [빠른 시작 가이드](quickstart.md)를 따라하면 설치부터 데이터 조회까지 한 번에 완료할 수 있습니다.
...
현재 지원 현황의 상세 정보와 진행 예정 항목은 [SUPPORTED_DATA.md](supported-data.md)에서 확인할 수 있습니다.
...
- [CANONICAL_MODEL.md](canonical-model.md): 다양한 API 응답을 하나로 통합하는 표준 데이터 모델 정의
- [PROVIDER_ADAPTER_CONTRACT.md](provider-adapter-contract.md): 새로운 데이터 제공 기관(Provider) 추가를 위한 어댑터 구현 규약

### API & 검증
- [API_SPEC.md](api-spec.md): 사용자가 직접 사용하는 파이썬 API 명세 및 사용법
...
- [SUPPORTED_DATA.md](supported-data.md): 지원 공공데이터 현황 및 진행 상태

### 상세 참고 자료
- [quickstart.md](quickstart.md): **처음 사용자를 위한 단계별 빠른 시작 가이드**
- [product-family-architecture.md](product-family-architecture.md): **KPubData 제품군 전체 시스템 아키텍처** (3개 저장소 관계도)
- [architecture-diagrams.md](architecture-diagrams.md): 아키텍처 시각화 다이어그램
- [datago-api-reference.md](datago-api-reference.md): 공공데이터포털(data.go.kr) API 연동 참고 자료
- [ADRs](adrs/): 주요 기술적 결정 이력 (Architecture Decision Records)
