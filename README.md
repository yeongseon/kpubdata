...
|---|---|---|---|
| 공공데이터포털 (`datago`) | 36개 | 단기예보, 대기오염, 부동산 실거래가 등 | 지원 |
| 지방행정인허가 (`localdata`) | 195개 | 일반음식점, 휴게음식점, 의원 등 | 지원 |
| 소상공인 (`semas`) | 17개 | 반경/사각형/다각형 내 상가정보 등 | 지원 |
| 지방재정365 (`lofin`) | 6개 | 세출결산, 세입결산, 재정자립도 등 | 지원 |
| 서울 열린데이터광장 (`seoul`) | 4개 | 지하철 실시간 도착, 따릉이 실시간 대여 등 | 지원 |
| 한국은행 ECOS (`bok`) | 3개 | 기준금리, 원/달러 환율, 국고채 3년물 수익률 | 지원 |
| 한국거래소 (`krx`) | 3개 | 코스피 지수, 투자자별 순매수, 밸류에이션 | 지원 |
| 한국거래소 (`pykrx`) | 3개 | 코스피 OHLCV, 코스닥 OHLCV, 종목별 시세 | 지원 |
