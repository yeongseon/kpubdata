...
| Provider | 대표 Dataset | 상태 |
|---|---|---|
| 공공데이터포털 (`datago`) | 단기예보, 대기오염, 부동산 실거래가 외 19개 | 지원 |
| 서울 열린데이터광장 (`seoul`) | 지하철 실시간 도착, 따릉이 월별 이용 외 2개 | 지원 |
| 한국은행 ECOS (`bok`) | 기준금리, 원/달러 환율, 국고채 3년물 수익률 | 지원 |
| 통계청 KOSIS (`kosis`) | 인구이동, 광공업생산지수 | 지원 |
| 지방재정365 (`lofin`) | 세출결산, 세입결산, 재정자립도 외 3개 | 지원 |
| 한국거래소 (`krx`) | 코스피 지수, 투자자별 순매수, 밸류에이션 | 지원 |
| 통계청 SGIS (`sgis`) | 시도/시군구/읍면동 행정구역 경계 | 지원 |
| 지방행정인허가 (`localdata`) | 일반음식점, 휴게음식점 | 지원 |
| 소상공인 (`semas`) | 반경 내 상가정보 | 지원 |
