...
[공공데이터포털(data.go.kr)](https://www.data.go.kr)에서 발급받은 API 키를 [환경 변수](https://ko.wikipedia.org/wiki/%ED%99%98%EA%B2%BD_%EB%B3%80%EC%88%98)(운영체제에 저장하는 설정값)로 설정하거나 코드에서 직접 전달합니다.
...
| 데이터셋 ID | 이름 | 설명 |
|---|---|---|
| `datago.village_fcst` | 단기예보 조회서비스 | 기상청 단기 예보 데이터 |
| `datago.ultra_srt_ncst` | 초단기실황 조회서비스 | 기상청 실시간 기상 실황 데이터 |
| `datago.air_quality` | 대기오염정보 조회서비스 | 실시간 미세먼지(PM2.5, PM10) 정보 |
| `datago.bus_arrival` | 경기도 버스도착정보 조회서비스 | 경기도 버스 도착 예정 정보 |
| `datago.hospital_info` | 병원정보서비스 | 전국의 병원 및 의료 기관 정보 조회 |
...
- 통계청([KOSIS](https://kosis.kr)), 한국은행([ECOS](https://ecos.bok.or.kr)) 등 신규 제공 기관 확대
