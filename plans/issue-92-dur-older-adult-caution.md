# DUR 노인주의 정보조회 추가 계획 (Issue #92)

## 범위
- 공공데이터포털(data.go.kr) 식품의약품안전처 의약품안전사용서비스(DUR)품목정보 API 중 `getOdsnAtentInfoList03` operation 하나만 추가한다.
- 데이터셋 키는 `dur_older_adult_caution`을 사용한다.
- 한국어 명칭은 API 목록의 `노인주의 정보조회`를 기준으로 문서와 카탈로그에 반영한다.
- 전체 DUR API를 한 번에 구현하지 않고, 기존 `dur_usjnt_taboo`와 같은 metadata-only 패턴을 따른다.
- `DataGoAdapter` 구조나 public API는 변경하지 않는다.

## 수정 대상
- `src/kpubdata/providers/datago/catalogue.json`
- `tests/fixtures/datago/success_dur_older_adult_caution.json`
