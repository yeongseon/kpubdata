# DUR 서방정분할주의 정보조회 추가 계획 (Issue #92)

## 범위
- 공공데이터포털(data.go.kr) 식품의약품안전처 의약품안전사용서비스(DUR)품목정보 API 중 `getSeobangjeongPartitnAtentInfoList03` operation 하나만 추가한다.
- 데이터셋 키는 `dur_er_tablet_split_caution`을 사용한다.
- 한국어 명칭은 API 목록의 `서방정분할주의 정보조회`를 기준으로 카탈로그, 지원 현황, PR 본문에 반영한다.
...
- `tests/fixtures/datago/success_dur_er_tablet_split_caution.json`
...
- Issue #92에는 남은 operation이 있으므로 PR 본문에는 이슈를 종료하는 키워드를 사용하지 않는다.
