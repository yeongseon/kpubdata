# DUR 투여기간주의 정보조회 추가 계획 (Issue #92)

## 범위

- 공공데이터포털(data.go.kr) 식품의약품안전처 의약품안전사용서비스(DUR)품목정보 API 중 `getMdctnPdAtentInfoList03` operation 하나만 추가한다.
- 데이터셋 키는 `dur_medication_period_caution`을 사용한다.
- 한국어 명칭은 API 목록의 `투여기간주의 정보조회`를 기준으로 카탈로그, 지원 현황, PR 본문에 반영한다.
- 전체 DUR API를 한 번에 구현하지 않고, 기존 `dur_usjnt_taboo`, `dur_older_adult_caution`과 같은 metadata-only 패턴을 따른다.
- 아직 main에 없는 `dur_product_info`, `dur_age_taboo`, `dur_dosage_caution` 변경에는 의존하지 않는다.

## 수정 대상

- `src/kpubdata/providers/datago/catalogue.json`
- `tests/fixtures/datago/success_dur_medication_period_caution.json`
- `tests/unit/providers/datago/test_fixtures.py`
- `tests/contract/test_datago.py`
- `SUPPORTED_DATA.md`
- `plans/issue-92-dur-medication-period-caution.md`
