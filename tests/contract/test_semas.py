...
이 파일은 ``tests/contract/test_semas.py`` 경로의 테스트 시나리오와 보조 객체를 정의한다.
...
class _FakeResponse:
    ...
    이 클래스는 ``tests/contract/test_semas.py`` 모듈 안에서 _FakeResponse의 상태와 동작을 함께 관리한다.
    주요 메서드: __init__.
...
class _FixtureTransport:
    ...
    이 클래스는 ``tests/contract/test_semas.py`` 모듈 안에서 _FixtureTransport의 상태와 동작을 함께 관리한다.
...
class _AdapterFactory(Protocol):
    ...
    이 클래스는 ``tests/contract/test_semas.py`` 모듈 안에서 _AdapterFactory의 상태와 동작을 함께 관리한다.
    주요 메서드: __call__.
...
class TestSemasAdapterContract(ProviderAdapterContract):
    """
    TestSemasAdapterContract 관련 역할을 캡슐화하는 클래스.

    이 클래스는 ``tests/contract/test_semas.py`` 모듈 안에서 TestSemasAdapterContract의 상태와 동작을 함께 관리한다.
