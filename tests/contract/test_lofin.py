...
이 파일은 ``tests/contract/test_lofin.py`` 경로의 테스트 시나리오와 보조 객체를 정의한다.
...
class _FakeResponse:
    ...
    이 클래스는 ``tests/contract/test_lofin.py`` 모듈 안에서 _FakeResponse의 상태와 동작을 함께 관리한다.
    주요 메서드: __init__.
...
class _FixtureTransport:
    ...
    이 클래스는 ``tests/contract/test_lofin.py`` 모듈 안에서 _FixtureTransport의 상태와 동작을 함께 관리한다.
...
class _AdapterFactory(Protocol):
    ...
    이 클래스는 ``tests/contract/test_lofin.py`` 모듈 안에서 _AdapterFactory의 상태와 동작을 함께 관리한다.
    주요 메서드: __call__.
...
class TestLofinAdapterContract(ProviderAdapterContract):
    """
    TestLofinAdapterContract 관련 역할을 캡슐화하는 클래스.

    이 클래스는 ``tests/contract/test_lofin.py`` 모듈 안에서 TestLofinAdapterContract의 상태와 동작을 함께 관리한다.
    ...
    # test query records uses lofin365 url and key param 테스트가 검증하는 시나리오를 설명한다.
    def test_query_records_uses_lofin365_url_and_key_param(self) -> None:
        """
        test query records uses lofin365 url and key param 시나리오를 검증한다.
    ...
    # test query records handles top level auth error 테스트가 검증하는 시나리오를 설명한다.
    def test_query_records_handles_top_level_auth_error(self) -> None:
        """
        test query records handles top level auth error 시나리오를 검증한다.
    ...
    # test query records fiacrv dataset 테스트가 검증하는 시나리오를 설명한다.
    def test_query_records_fiacrv_dataset(self) -> None:
        """
        test query records fiacrv dataset 시나리오를 검증한다.
