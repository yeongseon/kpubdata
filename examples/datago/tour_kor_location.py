...
"""datago.tour_kor_location 예제 — 자동 생성(배치 전환).

실행 모드:
- ``KPUBDATA_MODE=replay`` — fixture 재생(키 불필요) / 미지정 — 실호출
- 파라미터는 spec의 예제 ``gangnam_1km``와 동일(replay 매칭 계약)
...
def main() -> None:
    ...
    print(f"datago.tour_kor_location: {len(batch.items)}건 / 전체 {batch.total_count}건")
