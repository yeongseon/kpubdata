...
logger = logging.getLogger("kpubdata.provider.semas")
...
class SemasAdapter:
    ...
    def get_dataset(self, dataset_key: str) -> DatasetRef:
        ...
        logger.debug(
            "Semas dataset not found",
            extra={"dataset_id": f"semas.{dataset_key}", "provider": "semas"},
        )
        raise DatasetNotFoundError(
            f"Dataset not found: semas.{dataset_key}",
            provider="semas",
            dataset_id=f"semas.{dataset_key}",
        )
    ...
    def _build_request_url(self, dataset: DatasetRef, operation: str | None = None) -> str:
        ...
        if not isinstance(base_url_raw, str) or not base_url_raw:
            logger.debug(
                "Semas dataset metadata missing base_url", extra={"dataset_id": dataset.id}
            )
    ...
    def _request_and_decode(
        ...
        try:
            ...
            exc.provider = "semas"
            logger.debug("Semas response parsing failed", extra={"dataset_id": dataset_id})
            raise
        except ImportError as exc:
            raise ParseError("Failed to parse semas response", provider="semas") from exc

        if isinstance(decoded, dict):
            return cast(dict[str, object], decoded)

        logger.debug("Semas decoded payload invalid type", extra={"dataset_id": dataset_id})
        raise ParseError("Decoded payload is not an object", provider="semas")
    ...
    def _raise_for_result_code(self, code: str, msg: str, dataset_id: str) -> NoReturn:
        ...
        if code == "22":
            raise RateLimitError(msg, provider="semas", provider_code=code, retryable=False)
        if code == "10":
            raise InvalidRequestError(msg, provider="semas", provider_code=code)
        if code == "12":
            raise DatasetNotFoundError(
                msg,
                provider="semas",
        ...
        if code in {"01", "02"}:
            raise ServiceUnavailableError(msg, provider="semas", provider_code=code)
    ...
    def _normalize_items(self, items_wrapper: object) -> list[dict[str, object]]:
        ...
        if isinstance(items_wrapper, dict):
            ...
            if "item" in items_wrapper or not items_wrapper:
                ...
                # 생긴다.
                return []
            # 그 밖의 비어 있지 않은 dict 는 래핑 없이 온 단건이다.
